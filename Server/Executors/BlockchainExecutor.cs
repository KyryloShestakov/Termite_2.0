...
public class BlockchainExecutor
{
    ...
    public BlockchainExecutor(TcpClient tcpClient, TcpHandler tcpHandler, TcpRequest tcpRequest)
    {
        ...
        try
        {
            ...
            switch (request.RequestType)
            {
                case "Transaction":
                    try
                    {
                        Logger.Log("Processing transaction...", LogLevel.Information, Source.Server);
                ...
                case "Blocks":
                    try
                    {
                        Logger.Log("Processing blocks...", LogLevel.Information, Source.Server);
                        Response response = await _controller.HandleRequestAsync(request);
                        string responseJson = JsonSerializer.Serialize(response);
