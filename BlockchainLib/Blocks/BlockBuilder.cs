    ...
    public class BlockBuilder
    {
            ...
            try
            {
                if (await TryBuildBlock())
                {
                    if (await _blockChain.GetCountAsync() == 0)
                    {
                        ...
                        Logger.Log($"Genesis block created: {genesisBlock.Id}");
                        return genesisBlock;
                    }
                    
                    Block block = await BuildBlockAsync();
                    Logger.Log($"Block created: {block.Id}");
                    return block;
                }
                else
                {
        ...
        private async Task<Block> CreateGenesisBlock()
        {
            try
            {
                ...
                string previousHash = "previousBlockHash";
                string merkleRoot = "calculatedMerkleRoot";
                int difficulty = 2;
