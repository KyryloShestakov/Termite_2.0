...
namespace StorageLib.DB.SqlLite.Services
{
    public class PeerInfoService
    {
        ...
        public async Task<PeerInfoModel?> GetPeerInfoByIdAsync(int id)
        {
            return await _context.PeersInfo.FindAsync(id);
        }
        
        public async Task<bool> DeletePeerInfoAsync(int id)
        {
